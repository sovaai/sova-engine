...
#ifdef _MSC_VER
    #include <_include/_string.h> // bzero et al for Windows platforms
...
public:
    ...
    void free_buffer() { if(m_size) {__safe_avector_destruct(m_buffer, m_size); m_size=0; } if(m_buffer) free(m_buffer); m_buffer = NULL; m_alloc = 0; m_no_memory_flag = false; }
...
inline void avector<T>::remove(size_t pos)
{
    if(pos < m_size)
    {
        ...
        memmove(reinterpret_cast<void *>(m_buffer+pos),
                reinterpret_cast<const void *>(m_buffer+pos+1),
                (m_size-pos-1)* sizeof(T));
        m_size--;
    }
}
/*----------------------------------------------------------------------------*/
